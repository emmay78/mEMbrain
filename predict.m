function [density_mat, density_peak_location_mat] = ...
    ...
    i_init, i_final, j_init, j_final, z_init, z_final, num_classes, mask, mipPrediction)
...
predPath = fullfile(path_predictions,sprintf('mip%d',miplevel)); %fullfile('C:\Users\Elisa\Test', 'membranePred_unet_whole_volume_Olga_60B');
probPath =  fullfile(path_probabilities,sprintf('mip%d',miplevel)); %fullfile('C:\Users\Elisa\Test', 'membrane_probability_unet_whole_volume_Olga_60B');
mkdir(predPath);
mkdir(probPath);
...
for i = i_init:step:i_final
    ...
    for j = j_init:step:j_final
        ...
        xstart = max(i-pad_size,0);
        ystart = max(j-pad_size,0);
        xend = min(i+patch_size-1+pad_size, size_x_mip-1);
        yend = min(j+patch_size-1+pad_size, size_y_mip-1);
        ...
        while k <= z_final
            ...
            try
                [emimage]= vast.getemimage(layernr,miplevel,xstart,xend,ystart,yend,k,kend,getimmediatFlag,askvastLoad);
            catch
                keyboard
            end
            ...
            for ik=1:size(emimage,3)
                ...
                try
                    [predictions,~,probabilities] = semanticseg(emimage_ik, net);
